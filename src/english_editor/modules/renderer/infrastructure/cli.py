...
# @title 📄 cli.py — [Infrastructure / Adapters] Adaptador de Línea de Comandos
...
def main() -> None:
    ...
    try:
        for seg_str in args.segments:
            ...
            raw_segments.append(
                {"start_ms": start_sec * 1000.0, "end_ms": end_sec * 1000.0}
            )
    except ValueError:
        print(
            "❌ Error de formato: Los segmentos deben tener el formato 'inicio,fin' con números. Ej: 2.5,5.0",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(
        f"   Segmentos a procesar: {len(raw_segments)} | Padding de seguridad: {padding_ms}ms"
    )

...
def create_dummy_source(source_path: Path, duration_sec: int) -> None:
    ...
    cmd = [
        "ffmpeg", 
        "-f", "lavfi", "-i", f"testsrc=duration={duration_sec}:size=640x360:rate=30", # Pista de Video
        "-f", "lavfi", "-i", f"sine=frequency=1000:duration={duration_sec}",          # Pista de Audio
        "-c:v", "libx264", 
        "-c:a", "aac", 
        str(source_path), 
        "-y"
...
def execute_domain_demo() -> None:
    ...
    print(f"Segmento Original: {original.start_ms/1000}s -> {original.end_ms/1000}s (Duración: {original.duration_ms/1000}s)")
    print(f"Padding a aplicar: {pad.duration_ms/1000}s")
    print(f"Segmento Resultante: {padded.start_ms/1000}s -> {padded.end_ms/1000}s (Duración: {padded.duration_ms/1000}s)")
    ...
    print(f"✅ Regla DoD #3 verificada: El inicio se limitó matemáticamente a 0.0s.")
...
def execute_use_case_demo() -> None:
    ...
    dummy_dir = Path("/tmp/renderer_demo")
    ...
    print("Executando RenderMediaUseCase.execute()... (Llamada en vivo a FFmpeg complex-filter)...")

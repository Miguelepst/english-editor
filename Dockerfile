...
RUN apt-get update && apt-get install -y --no-install-recommends     ${APT_REQUIREMENTS}     && rm -rf /var/lib/apt/lists/*
...
# Instalación inyectada por Shift-Left Audit
ARG INSTALL_CMD="pip install --no-cache-dir --no-deps ."
RUN ${INSTALL_CMD}

USER appuser

# [OPCIÓN 2]: Inmortalidad Offline para Inteligencia Artificial

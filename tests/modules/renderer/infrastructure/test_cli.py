...
from english_editor.modules.renderer.infrastructure.cli import parse_args, main
...
def test_cli_main_should_convert_seconds_to_ms_and_execute_use_case(MockSplicer, MockUseCase):
    ...
    test_args = ["cli.py", "-s", "input.mp4", "-o", "output.mp4", "-t", "2.0,4.0", "7.0,9.0", "-p", "500"]
    ...
    assert call_kwargs["raw_segments"] == [{"start_ms": 2000.0, "end_ms": 4000.0}, {"start_ms": 7000.0, "end_ms": 9000.0}]

@patch("english_editor.modules.renderer.infrastructure.cli.RenderMediaUseCase")
@patch("english_editor.modules.renderer.infrastructure.cli.FFmpegMediaSplicer")
def test_cli_main_should_handle_domain_errors_gracefully(MockSplicer, MockUseCase, capsys):
    mock_use_case_instance = MagicMock()
    MockUseCase.return_value = mock_use_case_instance
    mock_use_case_instance.execute.side_effect = ValueError("El inicio no puede ser negativo")

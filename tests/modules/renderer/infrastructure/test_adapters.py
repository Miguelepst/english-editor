...
# @title 🧪 test_adapters.py — [Test] Adaptador FFmpeg

# ✅ Test creado: /content/english-editor/tests/modules/renderer/infrastructure/test_adapters.py
# 🧪 Ejecutar test: cd english-editor && python -m pytest tests/modules/renderer/infrastructure/test_adapters.py -v
...
def test_ffmpeg_splicer_should_construct_correct_command_and_return_path(
    ...
    result = splicer.splice_and_render(
        source_path=source, segments=segments, output_path=output
    )
...
@patch("english_editor.modules.renderer.infrastructure.adapters.subprocess.run")
@patch("english_editor.modules.renderer.infrastructure.adapters.Path.exists")
def test_ffmpeg_splicer_should_translate_subprocess_error_to_domain_error(
    mock_exists, mock_run
):
    """
